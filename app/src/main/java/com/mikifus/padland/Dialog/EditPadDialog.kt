...
open class EditPadDialog(title: String, callback: FormDialogCallBack) : FormDialog(title, callback) {
    ...
    override var contentValues: ContentValues? = null
    init {
    ...
    override fun setViewEvents() {
        lifecycleScope.launch {
            ...
            val allPadGroups = padGroupModel.getAllPadGroups()
            allPadGroups.add(PadGroup(context))
            spinnerAdapter = SpinnerGroupAdapter(
                requireContext(),
...
//        val prefix = serverModel.getServerPrefixFromUrl(context, pad.server)
        val prefix = PadListDatabase.getInstance(requireContext()).serverDao().getServerPrefixFromUrl(pad.server)
